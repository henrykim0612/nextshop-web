...
interface Props {
  ...
  setSelectedCartItemId: (value: SetStateAction<number | null>) => void;
}

export default function CartDeleteModal({ selectedCartItemId, setSelectedCartItemId }: Props) {
  const router = useRouter();

  const handleDeleteCartItem = async () => {
    const jwt = getJwt();
    if (jwt) {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/cart/${selectedCartItemId}`, {
        method: 'DELETE',
        headers: {
  ...
  return (
    <Dialog open={!!selectedCartItemId} onClose={() => setSelectedCartItemId(null)} className="relative z-10">
      <DialogBackdrop
        transition
              ...
              <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                ...
                <div className="mt-2">
                  <p className="text-sm text-gray-500 ">
                    Are you sure you want to remove this item from your cart? Once removed, it will be permanently
